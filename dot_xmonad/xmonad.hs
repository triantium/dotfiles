...
main :: IO ()
main = do {
        autorandr <- spawnPipe "autorandr --change";
        xmonproc1 <- spawnPipe "xmobar -x 1";
        xmonproc0 <- spawnPipe "xmobar -x 0";
        xmonad $ docks def {
      -- simple stuff
        terminal           = myTerminal,
      ...
      -- hooks, layouts
        ...
        logHook            = myLogHook <+> dynamicLogWithPP xmobarPP
                        { ppOutput = \x -> hPutStrLn xmonproc0 x  >> hPutStrLn xmonproc1 x  -- >> hPutStrLn xmproc2 x --Third monitor
